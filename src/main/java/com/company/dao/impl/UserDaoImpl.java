...
public class UserDaoImpl extends AbstractDao implements UserDaoInter {
    ...
    private User getUserSimple(ResultSet rs) throws Exception {
        ...
        User user = new User(id, name, surname, phone, email, address, profileDesc, birthdate, null, null);
    ...
    public boolean updateUser(User u) {
        try ( Connection c = connect()) {
            PreparedStatement stmt = c.prepareStatement("update user set name=?,surname=?,phone=?,email=?,address=?, profile_description=?, birthdate=?, birthplace_id=?  where id=?");
    ...
    public boolean addUser(User u) {
        try ( Connection c = connect()) {
            PreparedStatement stmt = c.prepareStatement("insert into user(name,surname,phone,email,address,password,profile_description) values(?,?,?,?,?,?,?)");
            stmt.setString(1, u.getName());
            stmt.setString(2, u.getSurname());
            stmt.setString(3, u.getPhone());
            stmt.setString(4, u.getEmail());
